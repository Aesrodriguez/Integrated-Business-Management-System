(function (global) {
  ...
  function buildTimeoutError(ms) {
    return new Error(
      "Tiempo de espera agotado (" + ms + "ms) al consultar la API REST",
    );
  }
  ...
  function callRest(method, payload) {
    ...
    if (
      method === "getDatosInicialesRapido" ||
      method === "getDatosIniciales"
    ) {
      return callProxy({
        endpoint: "/api/v1/trabajadores",
        httpMethod: "GET",
  ...
  function request(method, args, options) {
    ...
    var ttl =
      options && Object.prototype.hasOwnProperty.call(options, "ttlMs")
        ? options.ttlMs
        : config.requestTtlMs;
    var now = Date.now();
    ...
    var promise = (
      canUseAppsScript()
        ? callAppsScript(method, args)
        : callRest(method, (args || [])[0])
    )
      .then(function (result) {
  ...
  function loadViewData(view) {
    ...
    if (view === "items")
      return request("getItems", []).then(function (value) {
        store.mergeCache("items", value);
        return value;
      });
    if (view === "trabajadores")
      return request("getTrabajadores", [{ soloResumen: false }]).then(
        function (value) {
          store.mergeCache("workers", value);
          return value;
        },
      );
    if (view === "contratos")
      return request("getDashboardContratos", []).then(function (value) {
        store.mergeCache("contracts", value);
        return value;
      });
    if (view === "lista")
      return request("getCotizaciones", [{}]).then(function (value) {
        store.mergeCache("cotizaciones", value);
        return value;
      });
    if (view === "clientes")
      return request("getTrabajadores", [{ soloResumen: true }]);
  ...
  function setBasicCredentials(user, pass) {
    ...
    runtimeBasicAuthHeader =
      "Basic " + global.btoa(cleanUser + ":" + cleanPass);
    return true;
  }
